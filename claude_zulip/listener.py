"""Zulip listener for a Claude bot: zulipmcp's listener plus catch-up and limits.

zulipmcp.listener only sees @mentions that arrive while it is connected. When
the machine sleeps, the event queue dies and mentions posted in the gap are
never delivered, so nobody answers them. This wrapper, on every start, looks
back for mentions the bot has not answered and spawns a session for each, then
hands off to the stock listener. Same command line as
``python -m zulipmcp.listener``, plus ``--state-dir DIR`` (default
``~/.claude/channels/zulip``) and ``--limits FILE`` (default
``<state-dir>/limits.json``).

It also gates every spawn through ``usage_ledger``: per-sender and org-wide
limits, so nobody can use the bot as a free assistant or burn its human's
tokens for fun. A blocked mention gets a canned reply through the Zulip API
(no tokens) and the bot's human gets one ``[audit]`` DM per sender per hour.

INVARIANTS:
- A mention is "answered" only if the bot posted in that topic after it.
...
- Exempt users (the bot's human) are never blocked; blocked mentions never spawn.
"""
...
_state_file = ledger.STATE_DIR / "last_seen_id"
...
def _remember(msg_id: int) -> None:
    ...
    if msg_id > (_last_seen() or 0):
        _state_file.write_text(str(msg_id))
...
def _spawn_gated(cfg: stock.Config, msg: dict) -> None:
    ...
    limits = ledger.load_limits(ledger.LIMITS_PATH)
    ok, reason = ledger.check(msg["sender_id"], ledger.read_spawns(ledger.SPAWNS_PATH), time.time(), limits)
    if not ok:
    ...
    ledger.record_spawn(msg["sender_id"], msg.get("sender_full_name", "?"),
                        msg["display_recipient"], msg["subject"], str(log), path=ledger.SPAWNS_PATH)
...
def configure_state(state_dir: Path | None, limits: Path | None) -> None:
    """Point the ledger and the catch-up state at a directory."""
    global _state_file
    if state_dir is not None:
        ledger.STATE_DIR = state_dir
        ledger.SPAWNS_PATH = state_dir / "spawns.jsonl"
        ledger.LIMITS_PATH = state_dir / "limits.json"
        _state_file = state_dir / "last_seen_id"
    if limits is not None:
        ledger.LIMITS_PATH = limits
...
def main(argv: list[str] | None = None) -> None:
    ...
    state_dir = _take(args, "--state-dir")
    limits = _take(args, "--limits")
    configure_state(Path(state_dir).expanduser() if state_dir else None,
                    Path(limits).expanduser() if limits else None)
