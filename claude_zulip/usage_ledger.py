"""Usage ledger and abuse limits for a Claude bot's Zulip listener.
...
INVARIANTS:
- Exempt users (the bot's human) are never blocked.
...
LIMITS_PATH = STATE_DIR / "limits.json"
...
def check(sender_id: int, spawns: list[dict], now: float, limits: dict) -> tuple[bool, str]:
    ...
    if s["usd_day"] >= float(limits["per_sender_usd_per_day"]):
        return False, f"${s['usd_day']:.2f} of my human's budget today (limit ${float(limits['per_sender_usd_per_day']):.2f})"
...
def block_message(reason: str) -> str:
    return (f"I'm rate-limited right now: {reason}. I'm here to coordinate on my human's "
            "behalf, not as a general assistant, so your own Claude is the place for the rest. "
            "My human can raise the limit if this was a real need.")
